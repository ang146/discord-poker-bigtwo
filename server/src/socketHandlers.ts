...
import { rooms, getOrCreateRoom, findPlayer, removeFromAll, broadcast } from './rooms';
...
export function registerSocketHandlers(io: Server): void {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('player:sit', (payload: { roomId: string; userId: string }) => {
      ...
      if (room.players.length >= GAME_MAX_PLAYERS[room.selectedGame as GameId]) return;
    ...
    // ── Ready state ─────────────────────────────────────────────────────────
    socket.on('player:ready', (payload: { roomId: string; userId: string; ready: boolean }) => {
    ...
    socket.on('room:set-game', (payload: { roomId: string; game: GameId }) => {
      ...
      while (room.players.length > max) room.spectators.unshift(room.players.pop()!);
      broadcast(io, roomId);
    });

    socket.on('room:set-bots', (payload: { roomId: string; enabled: boolean }) => {
    ...
    socket.on('room:transfer-host', (payload: { roomId: string; toUserId: string }) => {
    ...
    socket.on('room:start-game', (payload: { roomId: string }) => {
      ...
      if (room.botsEnabled) {
        if (humanPlayers.length === 0 || !humanPlayers.every(p => p.isReady)) return;
      } else {
        if (room.players.length !== max || !room.players.every(p => p.isReady)) return;
      }
    ...
    socket.on('game:play', (payload: PlayCardsPayload) => {
      ...
      for (const pc of cards) {
        const idx = remaining.findIndex(c => c.rank === pc.rank && c.suit === pc.suit);
        if (idx === -1) {
